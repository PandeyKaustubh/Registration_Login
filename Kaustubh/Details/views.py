	...
	if request.method == 'POST' :
		...
		for x in obj :
			if(x.Username==data['Username'] and x.Password==data['Password']) :
				query4=dataTable.objects.filter(Username=data['Username']).values('link__Fname','link__Lname','link__Mobile_no','link__Email','link__School','link__HighestQualification','link__Gender','link__City','link__State','link__PinCode','link__Address')
				...
				return JsonResponse(list(query4),safe=False)
